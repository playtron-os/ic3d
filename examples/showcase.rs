...
//! the ground. The camera orbits automatically.
//!
//! ```bash
...
impl Scene3DProgram for ShowcaseScene {
    ...
    fn setup(&self, bounds: iced::Rectangle) -> Scene3DSetup {
        ...
        // Orbit camera — elevated, looking slightly down at the scene
        ...
        let objects: Vec<(Mesh, [f32; 4], Vec3, f32)> = vec![
            // (mesh, color+shininess, base_position, height_offset)
            (
                Mesh::cube(1.0),
                [0.85, 0.20, 0.18, 48.0],
        ...
        for (i, (mesh, color, base_pos, _height)) in objects.into_iter().enumerate() {
